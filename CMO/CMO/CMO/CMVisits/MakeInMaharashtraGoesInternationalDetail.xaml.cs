...
namespace CMO
{
	public partial class MakeInMaharashtraGoesInternationalDetail : ContentPage
	{
        ...
        public MakeInMaharashtraGoesInternationalDetail(string _countrytitle, string InternationalVisitItemTitle)
        {
            InitializeComponent();
            NavigationPage.SetBackButtonTitle(this, "");
            Title = InternationalVisitItemTitle.ToUpper();
            CountryTitle = _countrytitle;
            setfontsize();
        }
        ...
        public async Task CallWebServiceForForiegnVisits(string countrytitle)
        {
            ...
            else
            {
                try
                {
                    ...
                    values.Add(new KeyValuePair<string, string>("page_id", countrytitle));
                    ...
                    #endregion
                    var response = await GeneralClass.GetResponse<CMO.ServicesClasses.RootObjectCMVisitList>(CMO.ServiceLayer.ServiceLinks.international_DetailUrl, values);
                    if (response != null)
                    {
