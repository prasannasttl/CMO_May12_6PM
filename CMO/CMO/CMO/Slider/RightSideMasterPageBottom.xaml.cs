...
{
    public partial class RightSideMasterPageBottom : SlideMenuView, INotifyPropertyChanged
    {
        // ObservableCollection<ApplicationData> ApplicattionList;
        public static string NavigateToApp_PckgName = "";
        public RightSideMasterPageBottom()
        {
            InitializeComponent();
            this.IsFullScreen = true;
            this.MenuOrientations = MenuOrientation.BottomToTop;
            this.HeightRequest = App.DeviceHeight - 85;
            //  this.DraggerButtonHeight = _draggerHeight;
        ...
        {
            this.DraggerButtonHeight = this.HeightRequest / 2;
            this.HeightRequest = this.HeightRequest;
         //   ImageFullPath = this.IsShown;
        }
        public void SliderTapped(object sender, EventArgs args)
        {
            //this.Focus();
            //var x = this.IsFocused;
            ...
            for (int i = 0; i < ApplicattionList.Count; i++)
            {
                if (TargetPlatform.Android == Device.OS)
                {
                    ...
                    if (Device.Idiom == TargetIdiom.Phone)
                        ApplicattionList[i].SetButtonSize = 80;
                    else
                        ApplicattionList[i].SetButtonSize = 150;
                }
                ...
                if (TargetPlatform.iOS == Device.OS)
                {
                    ...
                    if (ApplicattionList[i].URL == "") { ApplicattionList[i].AppStatus = "OPEN"; }
                    if (Device.Idiom == TargetIdiom.Phone)
        ...
        public void AppStatusButtonCLicked(object sender, EventArgs args)
        {
            ...
            if (Device.OS == TargetPlatform.Android)
            {
                ...
                if (buttontext == "OPEN")
                {
                    if ((packgname == "com.MahaCMO") || (packgname == "com.MahaCMO.V2") || (packgname == "com.MahaCMO.V1"))
					{
            ...
            //ApplicattionList[5].AppStatus = "OPEN";
            //ApplicattionList[1].AppStatus = "OPEN";
            //ApplicattionList[2].AppStatus = "OPEN";
            //ApplicattionList[3].AppStatus = "OPEN";
            //AppList.ItemsSource = null;
            //AppList.ItemsSource = ApplicattionList;
        ...
        #region Shown Property Changes
        private bool imageFullPath;
        protected void OnPropertyChanged(PropertyChangedEventArgs e)
        {
            PropertyChangedEventHandler handler = PropertyChanged;
            if (handler != null)
                handler(this, e);
            if (ImageFullPath == true)
            {
                flipper.Rotation = 180;
            }
            else
            {
                flipper.Rotation = 180;
        ...
        protected void xOnPropertyChanged(string propertyName)
        {
            OnPropertyChanged(new PropertyChangedEventArgs(propertyName));
        }

        public bool ImageFullPath
        {
            ...
            set
            {
                ...
                {
                    imageFullPath = value;
                    xOnPropertyChanged("ImageFullPath");
                }
            }
        }
